...
<template>
  <Main class="grid gap-y-4">
    ...
    <Card
      class="grid max-w-screen-xl grid-cols-[var(--image-size)_1fr] grid-rows-[max-content,1fr] gap-8 justify-self-center bg-primary/5 p-8 [--image-size:400px] [--service-color:#F50004]"
    >
      <div
        class="grid grid-cols-[var(--icon-size)_1fr] gap-x-2 text-[var(--service-color)] [--icon-size:28px]"
      >
        <i
          class="bx bx-envelope row-span-2 row-start-2 text-[length:var(--icon-size)] leading-none"
        />
        <span class="col-start-2 text-xs/5 font-bold text-muted-foreground">COMING THIS 2025</span>
        <h2 class="text-[length:var(--icon-size)] font-bold leading-none">Email Marketing</h2>
        <p class="text-xs text-muted-foreground">Manage your email and all</p>
      </div>
      <img
        src="@/assets/undraw_email_campaign.svg"
        alt=""
        class="row-start-2 size-[var(--image-size)] justify-self-end"
      />
      <ScrollArea as="ul" class="row-span-2 max-h-[500px] [&>div>div]:grid [&>div>div]:gap-y-4">
