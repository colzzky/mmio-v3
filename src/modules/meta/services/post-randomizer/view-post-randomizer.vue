                ...
                <div v-if="campaignInterface !== CampaignInterface.INFO_UPDATE" class="space-y-5">
                    <div class="space-y-5">
                        <div class="flex justify-start space-x-2 items-center">
                            <Button variant="ghost" class="flex" @click="router.go(-1)"><i
                                    class="material-icons text-md">arrow_back</i></Button>
                            <span class="font-bold text-xl">{{ campaign.data.name }}</span>
                        </div>
                        <div class="flex justify-start">
                            <Button variant="outline" class="rounded-r-none border-r-0 w-[20vh]"
                                :class="{ 'bg-gray-100 font-bold': campaignInterface === CampaignInterface.INFO_VIEW }"
                                @click="campaignInterface = CampaignInterface.INFO_VIEW">Campaign
                                Details</Button>
                            <Button variant="outline" class="rounded-l-none w-[20vh]"
                                :class="{ 'bg-gray-100 font-bold': campaignInterface === CampaignInterface.POST_UPDATE }"
                                @click="campaignInterface = CampaignInterface.POST_UPDATE">Campaign
                                Contents/Post</Button>
                        </div>
                    </div>

                    <div v-if="campaignInterface === CampaignInterface.INFO_VIEW"
                        class="bg-gray-100 rounded-lg border border-gray-200">
                        <div class="flex flex-col gap-y-4 p-4">
                            <div class="flex justify-between">
                                <span class="font-bold text-lg">Campaign Information</span>
                                <Button variant="outline" class="rounded-r-none border-r-0 w-[20vh]"
                                    @click="campaignInterface = CampaignInterface.INFO_UPDATE">Edit Campaign
                                    Information</Button>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div class="flex flex-col gap-4">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <span class="">{{ campaign.data.timezone ?
                                            find_timezone(campaign.data.timezone)?.text : 'Not yet added' }}</span>
                                    </div>
                                    <div class="flex flex-col">
                                        <span class="font-bold">Frequency</span>
                                        <span class="">{{ campaign.data.frequency }}</span>
                                    </div>
                                    <div v-if="campaign.data.frequency === 'Weekly' || campaign.data.frequency === 'Monthly'"
                                        class="flex flex-col">
                                        <span class="font-bold">Weekly</span>

                                        <div class="flex w-full flex-row flex-wrap items-center gap-2 py-2">
                                            <div v-for="(day, index) in campaign.data.weekly" :key="day"
                                                class="flex items-center space-x-2 self-start rounded-full bg-gray-200 border border-gray-300 p-1 px-3 text-xs">
                                    ...
                                    <div v-if="campaign.data.frequency === 'Monthly'" class="flex flex-col">
                                        ...
                                        <div class="flex w-full flex-row flex-wrap items-center gap-2 py-2">
                                            <div v-for="(month, index) in campaign.data.monthly" :key="month"
                                                class="flex items-center space-x-2 self-start rounded-full bg-gray-200 border border-gray-300 p-1 px-3 text-xs">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <div v-if="campaign.data.time.length > 0">
                                            <div class="flex w-full flex-row flex-wrap items-center gap-2 py-2">
                                                <div v-for="(time, index) in campaign.data.time" :key="time"
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <div v-if="campaign.data.startDate && campaign.data.endDate">
                                            <span class="">{{ new Date(campaign.data.startDate).toDateString() }} - {{
