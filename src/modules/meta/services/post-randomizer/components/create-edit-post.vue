...
import { post_randomizer_posts_data, post_randomizer_service_data } from '@/core/types/WorkSpaceTypes'
...
interface ModalInterface extends Omit<Modal, 'open'> {
    open(
        args:
            | { intent: 'create'; prId: PostRandomizerServiceData['pr_id']; postData: PostRandomizerPostsData | null }
            | { intent: 'edit'; prId: PostRandomizerServiceData['pr_id']; postData: PostRandomizerPostsData | null },
    ): void
...
const modal = reactive<ModalInterface>({
    ...
    async open(args) {
        try {
            ...
            post_randomizer_post.value = this.postData ? this.postData : JSON.parse(JSON.stringify(post_randomizer_posts_data))
    ...
    async createPost() {
        if (post_randomizer_post.value) {
            ...
            post_randomizer_post_md.data.createdBy = current_member.data ? current_member.data.uid : active_workspace.data ? active_workspace.data.owner_uid : ''
        ...
        <DialogContent class="gap-y-4">
            <DialogHeader>
                <template v-if="modal.intent === 'create'">
                    ...
                    <DialogDescription> Enter the post randomizer Name and we'll redirect you after.
                    </DialogDescription>
                </template>
                <template v-else>
            ...
            <div class="flex flex-col gap-y-2">
                ...
                <div v-if="content_form.errors.postName" for="name"
            ...
            <div class="flex flex-col gap-y-2">
                ...
                <Textarea v-model="content_form.inputs.content" placeholder="What do you want to post?" required />
                <div v-if="content_form.errors.content" for="name" class="flex items-center gap-1 text-xs text-red-500">
                    <i class="material-icons text-sm">error</i>
            ...
            <div class="flex flex-col gap-y-2">
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 ">
                    <div class="flex flex-col items-center justify-center text-center relative"
                        @drop.prevent="handleDrop" @dragover.prevent="isDragging = true" @dragleave="isDragging = false"
                    ...
                    <ul v-if="files.length > 0" class="mt-4 w-full">
                        <li v-for="(file, index) in files" :key="index"
                            class="flex items-center justify-between bg-gray-50 border rounded-md p-2 mb-2">
                            <span class="truncate">{{ file.name }}</span>
                            <Button variant="ghost" @click="removeFile(index)" class="text-red-500 hover:text-red-700">
                                Remove
                            </Button>
                        </li>
                    </ul>
            ...
            <DialogFooter>
                ...
                <Button v-else-if="modal.intent === 'edit' && !modal.submitLoad" @click="modal.submitForm()">
                    Update
                </Button>
                <Button v-else-if="modal.submitLoad" variant="outline" size="xs" disabled
