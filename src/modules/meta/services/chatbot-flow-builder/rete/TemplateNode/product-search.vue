...
const node = ref<Node<'product_search_node'> | null>(null)

onMounted(() => {
  node.value = props.data as Node<'product_search_node'>
})
    ...
    <NodeCard :data-selected="data.selected" class="flex flex-col gap-y-3 pb-0">
      ...
      <section class="space-y-1">
        <div class="space-y-1 px-5">
          <div class="font-bold">Product Search</div>
          <div class="max-h-47 rounded-lg border border-gray-200 bg-white p-3">
            ...
            <div v-else>
              <p class="text-gray-300">No Product Search Available</p>
            </div>
          </div>
          <p class></p>
        </div>
