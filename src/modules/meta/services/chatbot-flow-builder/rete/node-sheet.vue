...
defineProps<{ data: Node<keyof NodeType> }>()

const nodeTypeMappings: Record<string, { icon: string; label: string }> = {
  message_node: {
    icon: 'bx:message',
    label: 'Message',
  },
  ...
  <Sheet :modal="false">
    ...
    <SheetContent side="left" class="p-0">
      <template v-if="sheetState === 'default'">
        ...
        >
          <Icon
            :icon="nodeTypeMappings[data.label].icon"
          ...
          <SheetDescription class="leading-none">
            {{ nodeTypeMappings[data.label].label }}
          </SheetDescription>
        </SheetHeader>
        <main class="grid gap-y-4 px-6 py-3">
          ...
          <div>
            ...
            <Textarea id="text" name="text" rows="5" class="resize-none" />
          </div>
      ...
      <template
        v-else-if="sheetState === 'create-quick-reply' || sheetState === 'edit-quick-reply'"
      >
        <SheetHeader
          class="grid grid-cols-[var(--icon-size),1fr] grid-rows-2 gap-x-3 gap-y-1.5 border-b-2 px-6 pb-3 pt-4 [--icon-size:theme(spacing.6)]"
        >
