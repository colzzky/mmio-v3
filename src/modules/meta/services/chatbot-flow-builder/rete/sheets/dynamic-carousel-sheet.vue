...
const localNodeData = ref<Node<'dynamic_carousel_node'> | undefined>(undefined)

onMounted(() => {
  const node = rete_init.editor?.getNode(active_flow.selected_node_id)
  if (!node) throw new Error('No Node found with the given ID')

  localNodeData.value = node as Node<'dynamic_carousel_node'>
})
  ...
  <template v-if="localNodeData && localNodeData.data">
    ...
    <template v-if="sheetState === 'default'">
      ...
      >
        ...
        <SheetDescription class="leading-none">Dynamic Carousel</SheetDescription>
      </SheetHeader>
