...
<template>
        ...
        <div class="flex items-center justify-between gap-x-2">
          ...
          <div class="flex gap-x-1">
            ...
            <Button size="icon" variant="ghost">
              <Icon
                @click="active_flow.ui.minimizeMenuPanel()"
                icon="mdi:arrow-vertical-collapse"
                class="size-5"
              />
            </Button>
          </div>
        ...
        <TabsContent value="node-list">
          ...
          >
            <Button
              ...
              variant="ghost"
              :data-selected="active_flow.selected_node_id === node.id"
  ...
  <transition name="slide-fade" mode="out-in">
    <div v-if="active_flow.ui.menuPanelMinimized" class="fixed left-0 top-0 p-4">
      <div
        class="w-[clamp(350px,100%,15%)] gap-y-0 overflow-hidden overflow-y-scroll rounded-lg bg-white p-0 shadow-none [&>button]:hidden"
      >
        <header class="grid gap-y-1.5 border-b p-2 text-sm">
          <div class="flex items-center justify-between gap-x-2">
            <div class="flex items-center gap-2">
              ...
              <Icon icon="bxl:dev-to" class="me-auto size-10" />
              <div class="">
                <h1 class="w-40 truncate ps-1.5 font-bold leading-none">
                  Flow Name Marketing Master IO Beta 1
                </h1>
                <h2 class="w-40 truncate ps-1.5 text-xs leading-none">
                  This is a test about marketing master io
                </h2>
              </div>
            </div>
