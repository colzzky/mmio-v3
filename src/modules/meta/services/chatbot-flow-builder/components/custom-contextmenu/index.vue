<template>
  <div class="min-w-48 rounded-xl border-2 border-neutral-200 bg-neutral-100 p-2 text-sm shadow-sm"
    @mouseover="hide.cancel" @mouseleave="hide" data-testid="context-menu" rete-context-menu @dblclick.stop="">
    <div v-if="searchBar" class="relative mb-2 rounded-md bg-white [--icon-size:theme(spacing.5)]">
      <Icon icon="bx:search" class="absolute left-0 top-1/2 size-[var(--icon-size)] -translate-y-1/2 translate-x-2" />
      <input v-model="filter" type="search" placeholder="Search Template..."
    ...
    <div class="max-h-[40vh] overflow-auto">
      <ContextMenuItem v-for="item in getItems" :key="item.key" :sub-items="item.subItems" :label="item.label" :delay
        @select="handler($event, item)" @hide="typeof onHide === 'function' ? onHide : () => { }" />
    </div>
  </div>
</template>
