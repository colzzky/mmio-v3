...
const changePasswordField = reactive<changePasswordField>({
    ...
    async validateSubmit(): Promise<void> {
        ...
        const validateWithPasswordConfirmation = ((dataInput: InputStructure) => {
            const passwordConfirmation = schema.refine((data) => data.newPassword === data.confirmPassword, {
    ...
    async changePasswordFirebase(): Promise<void> {
        ...
        if (user_auth.data) {
            ...
            const reauthenticate = await reauthenticateWithCredential(user_auth.data, credential).then(async () => {
    ...
    <div v-if="!componentLoad">
        <div class="flex flex-col gap-y-2">
            ...
            <div class="flex items-center justify-between text-sm">
                ...
                <Button variant="link" class="h-[unset] p-0 text-blue-500"
                    @click="changePasswordField.openCloseModal()">Change Password</Button>
            </div>
        </div>
        ...
        <Dialog v-model:open="changePasswordField.changePasswordModal">
            <DialogContent @interact-outside="(e) => e.preventDefault()">
                <div class="relative">
                    <button :disabled="changePasswordField.isLoading" @click="changePasswordField.openCloseModal()"
                ...
                <div>
                    ...
                    <div class="flex flex-col gap-y-3">
                        <div class="flex flex-col gap-y-2">
                            ...
                            <div class="flex gap-2 items-center">
                                <Input :disabled="changePasswordField.isLoading"
                                    v-model="changePasswordField.dataInput.oldPassword" type="password"
                                    name="oldPassword" id="oldPassword" placeholder="password" class="h-7 text-xs"
                                    @blur="changePasswordField.validateSingleField('oldPassword')" />
                            </div>
                            <div v-if="changePasswordField.errors.oldPassword" for="name"
                        ...
                        <div class="flex flex-col gap-y-2">
                            ...
                            <div class="flex gap-2 items-center">
                                <Input :disabled="changePasswordField.isLoading"
                                    v-model="changePasswordField.dataInput.newPassword" type="password"
                                    name="newPassword" id="newPassword" placeholder="password" class="h-7 text-xs"
                                    @blur="changePasswordField.validateSingleField('newPassword')" />
                            </div>
                            <div v-if="changePasswordField.errors.newPassword" for="name"
                                class="text-xs text-red-500 flex items-center gap-1">
                                <i class="material-icons text-sm">error</i>
                        ...
                        <div class="flex flex-col gap-y-2">
                            ...
                            <div class="flex gap-2 items-center">
                                <Input :disabled="changePasswordField.isLoading"
                                    v-model="changePasswordField.dataInput.confirmPassword" type="password"
                                    name="confirmPassword" id="confirmPassword" placeholder="password"
                                    class="h-7 text-xs"
                                    @blur="changePasswordField.validateSingleField('confirmPassword')" />
                            </div>
                            <div v-if="changePasswordField.errors.confirmPassword" for="name"
                                class="text-xs text-red-500 flex items-center gap-1">
                                <i class="material-icons text-sm">error</i>
                    ...
                    <DialogFooter class="pt-4">
                        <div>
                            <div v-if="!changePasswordField.isLoading" class="flex items-center justify-end gap-2">
