...
import { postCollection, postCollectionBatch } from '@/core/utils/firebase-collections'
...
const create_team_modal = reactive({
  ...
  async createNewTeam(): Promise<boolean> {
    if (user_auth.data) {
      ...
      const post = await postCollection(DbCollections.teams, {
        data: {...team_model.data,tm_id:crypto.randomUUID()} ,
        idKey: 'tm_id'
      })
      ...
      if (post.status && post.data) {
        ...
        await postCollection(DbCollections.team_members, {
  ...
  async inviteMembers(): Promise<void> {
    ...
    const add_members = await postCollectionBatch(DbCollections.team_members,
      {
        $sub_params: {
          tm_id: team_model.data.tm_id
        },
    ...
    const send_invites = await postCollectionBatch(DbCollections.invitations,
      {
        $sub_params: null,
        idKey:'iv_id',
        data: member_invite
